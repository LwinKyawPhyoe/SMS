      ...
      <div class="col-lg-5 col-md-12" style="padding:0px;">
        <div class="card">
          ...
          <div class="card-body" style="padding:1rem 0;border-bottom: 1px solid #8080808c;">
            ...
            <label v-if="savebutton == true" style="margin-left:50px;">
              Please Activate Academic Year
              <strong>*</strong>
            </label>
            <div class="col-12 column-12">
              <button
                class="save"
                id="globalSave"
                @click="addExam(SessionList.id)"
                v-for="SessionList in SessionList"
                :key="SessionList.id"
              >Save</button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    getAcademic() {
      this.axios.get("/api/activeacademicyr").then(response => {
    ...
    goEdit(id) {
      this.axios.get(`/api/exams/editexam/${id}`).then(response => {
        this.saveexam = response.data;
      });
    },
    goDelete(id) {
      this.axios.get(`/api/exams/deleteexam/${id}`).then(response => {
