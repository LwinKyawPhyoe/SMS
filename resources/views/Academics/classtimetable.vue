      ...
      <div class="card-body">
        <div class="row" id="row" style="margin: 0px;">
          <div class="col-lg-4 col-md-4 col-12 textbox">
            ...
            <select
              id="classid"
              @change="changeClass(ClassTimeTableObj.class_id)"
              class="inputbox"
              name="class"
              v-model="ClassTimeTableObj.class_id"
            >
              <option
          ...
          <div class="col-lg-4 col-md-4 col-12 textbox">
            ...
            <select
              id="sectionid"
              @change="changeSection(ClassTimeTableObj.section_id)"
              class="inputbox"
              name="class"
              v-model="ClassTimeTableObj.section_id"
            >
              <option
          ...
          <div class="col-lg-4 col-md-4 col-12 textbox">
            ...
            <select
              id="subjectid"
              @change="changeSubject(ClassTimeTableObj.subject_id)"
              class="inputbox"
              name="class"
              v-model="ClassTimeTableObj.subject_id"
            >
              <option
...
export default {
  ...
  created() {
    ...
    EventBus.$on("SessionSaved", response => {
      ...
      this.SubjectList = [
        { id: 0, name: "Select Subject", type: "", staff_id: "" }
      ];
      this.AllSubjectList = [];
  ...
  methods: {
    ...
    changeSection(aId) {
      this.SubjectList = [
        { id: 0, name: "Select Subject", type: "", staff_id: 0 }
      ];
      this.axios
        .post("/api/AssSubject/search", this.ClassTimeTableObj)
        .then(response => {
          for (let i = 0; i < response.data.length; i++) {
            for (let s = 0; s < this.AllSubjectList.length; s++) {
              if (response.data[i].subject_id == this.AllSubjectList[s].id) {
                this.SubjectList.push({
                  id: this.AllSubjectList[s].id,
                  name: this.AllSubjectList[s].name,
                  type: this.AllSubjectList[s].type,
                  staff_id: response.data[i].staff_id
                });
              }
            }
    ...
    checkValidate() {
      ...
      if (
        this.ClassTimeTableObj.class_id == "" ||
        this.ClassTimeTableObj.class_id == undefined ||
        this.ClassTimeTableObj.class_id == 0
      ) {
      ...
      if (
        this.ClassTimeTableObj.section_id == "" ||
        this.ClassTimeTableObj.section_id == undefined ||
        this.ClassTimeTableObj.section_id == 0
      ) {
      ...
      if (
        this.ClassTimeTableObj.subject_id == "" ||
        this.ClassTimeTableObj.subject_id == undefined ||
        this.ClassTimeTableObj.subject_id == 0
      ) {
    ...
    goSearch() {
      if (this.checkValidate()) {
        this.axios
          .post("api/ClassTimeTable/search", this.ClassTimeTableObj)
          .then(response => {
              ...
              for (let a = 0; a < this.classTimeTbl.length; a++) {
                if (this.classTimeTbl[a].day == response.data[i].day) {
                  this.classTimeTbl[a] = {
                    ...
                    time_from: response.data[i].time_from,
                    time_to: response.data[i].time_to,
                    room_no: response.data[i].room_no
                  };
                }
              }
            }
    ...
    goSave() {
      ...
      this.axios
        .post("/api/ClassTimeTable/save", this.ClassTimeTableObj)
        .then(response => {
