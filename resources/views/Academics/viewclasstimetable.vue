            ...
            <tbody>
              <tr v-for="item in ClassTimeTblList" :key="item.subjectname" class="active">
                ...
                <td v-for="timetbl in item.timetbllist" :key="timetbl.id" class="all" nowrap>
                  ...
                  >{{ getTime(timetbl.time_from, timetbl.time_to) }}</label>
                  <label
                    style="display: block; margin: 0px;"
                  >{{ getRoomNo(timetbl.time_from, timetbl.time_to, timetbl.room_no) }}</label>
                </td>
              </tr>
            </tbody>
          </table>
  ...
  methods: {
    ...
    goSearch() {
      if (this.checkValidate()) {
        this.axios
          .post("api/ClassTimeTable/viewsearch", this.ClassTimeTableObj)
          .then(response => {
    ...
    sortClassTimeTblList(aList) {
      ...
      for (let i = 0; i < aList.length; i++) {
        if (this.ClassTimeTblList == [] || this.ClassTimeTblList.length == 0) {
          let timelist = [];
          timelist.push({
            id: aList[i].classTimeTblid,
            day: aList[i].day,
            time_from: aList[i].time_from,
            time_to: aList[i].time_to,
            room_no: aList[i].room_no
          });
          this.ClassTimeTblList.push({
            subjectname: aList[i].name,
            timetbllist: timelist
          });
        } else {
          let check = 0;
          for (let a = 0; a < this.ClassTimeTblList.length; a++) {
            if (this.ClassTimeTblList[a].subjectname == aList[i].name) {
              this.ClassTimeTblList[a].timetbllist.push({
                id: aList[i].classTimeTblid,
                day: aList[i].day,
                time_from: aList[i].time_from,
                time_to: aList[i].time_to,
