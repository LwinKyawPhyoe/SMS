          ...
          <div class="card-body" style="padding:1rem 0;border-bottom: 1px solid #8080808c;">
            ...
            <form @submit.prevent="goSave">
              <div class="col-lg-12 col-md-6 col-12">
                ...
                <select
                  id="classid"
                  @change="changeClass(ClassTeacher.class_id)"
                  class="inputbox"
                  name="class"
                  v-model="ClassTeacher.class_id"
                >
                  <option
              ...
              <div class="col-lg-12 col-md-6 col-12">
                ...
                <select
                  id="sectionid"
                  @change="changeSection(ClassTeacher.section_id)"
                  class="inputbox"
                  name="class"
                  v-model="ClassTeacher.section_id"
                >
                  <option
                ...
                <div v-for="teacher in TeacherList" :key="teacher.id" class="checkbox">
                  <label class="checkboxlabel">
                    <input
                      ...
                      @click="click_checkbox($event, teacher)"
                    />
                    {{ teacher.name }}
                  </label>
                </div>
...
export default {
      ...
      ClassList: [
        {
          id: 0,
          class: "Select Class",
          section: [{ id: 0, section: "Select Section" }]
        }
      ],
      SectionList: [{ id: 0, section: "Select Section" }],
      TeacherList: [
        { id: 1, name: "Shivam", checked: false },
        { id: 2, name: "Jason", checked: false }
      ],
      AssClassTeacherList: [],
      classsection: [],
      props: {
  ...
  methods: {
    ...
    getAssClassTeacher() {
      ...
      this.axios.get("/api/classTeacher").then(response => {
        for (let i = 0; i < response.data.length; i++) {
          ...
          this.AssClassTeacherList.push({
            id: response.data[i].id,
            class_id: response.data[i].class_id,
            staff_id: staffAry,
            section_id: response.data[i].section_id
          });
        }
      });
    },
    ...
    checkValidate() {
      ...
      if (
        this.ClassTeacher.class_id == "" ||
        this.ClassTeacher.class_id == undefined ||
        this.ClassTeacher.class_id == 0
      ) {
      ...
      if (
        this.ClassTeacher.section_id == "" ||
        this.ClassTeacher.section_id == undefined ||
        this.ClassTeacher.section_id == 0
      ) {
