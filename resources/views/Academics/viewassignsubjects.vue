<template>
    <div class="form" id="bar">
        <div class="toplink">
            ...
            <h4 class="stuLink">
                <a href="" style="text-decoration:none;color:#1b5e20;">Home </a> > Assign Subjects
            </h4>
        </div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <router-link to="/asssubjects" onclick="showForm()" class="add">
                    Add
                </router-link>
            </div>
            <div class="card-body">                
                        ...
                        <tbody>
                            <tr v-for="obj in AssSubList" :key="obj.id" class="active">
                                ...
                                <td style="text-align:right;vertical-align:top;">
                                    ...
                                    <i @click="goDelete(obj.AssignSubObj)" data-toggle="modal" data-target="#exampleModalCenter" class="fa fa-trash time">
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content modal-media-content">
                    ...
                    <div class="modal-body" style="padding: 10px 0px;">
                        <div class="row" id="row" style="margin: 0px;">
                            <div class="col-lg-6 col-12 textbox">
                                <div class="col-lg-3 col-12" style="display: inline-block;padding: 0px;margin: 0px;">
                                    <label for="name" class="title">Class</label>
                                </div>
                                <div class="col-lg-7 col-12" style="display: inline-block;padding: 0px;margin: 0px;">
                        ...
                        <div v-for="assSub in AssSubObj" :key="assSub.id" class="row" id="row" style="margin: 0px;">
                            <div class="col-lg-6 col-12 textbox">
                                <div class="col-lg-3 col-12" style="display: inline-block;padding: 0px;margin: 0px;">
                    ...
                    <div class="modal-footer" style="padding: 10px 15px;">
                        <button class="save btn-dark" id="globalSave" style="margin: 0px;" data-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    methods:{
        ...
        AssSubSort(alist){
            ...
            for(let i=0; i<alist.length; i++){
                if(this.AssSubList == [] || this.AssSubList.length == 0){
                    let asssubobj = [];
                    asssubobj.push({id: alist[i].AssSubId, SubId: alist[i].subject_id, StaffId: alist[i].staff_id});
                    this.AssSubList.push({classid: alist[i].classid, class: alist[i].class, sectionid: alist[i].sectionid, section: alist[i].section, AssignSubObj: asssubobj});
                }
                else
                {
                    let acheck = 0;
                    for(let a=0; a < this.AssSubList.length; a++){
                        if(this.AssSubList[a].classid == alist[i].classid && this.AssSubList[a].sectionid == alist[i].sectionid){
                            this.AssSubList[a].AssignSubObj.push({id: alist[i].AssSubId, SubId: alist[i].subject_id, StaffId: alist[i].staff_id});
                            acheck = 1;
                        }
                    }
                    ...
                    if(acheck == 0){
                        let asssubobj1 = [];
                        asssubobj1.push({id: alist[i].AssSubId, SubId: alist[i].subject_id, StaffId: alist[i].staff_id});
                        this.AssSubList.push({classid: alist[i].classid, class: alist[i].class, sectionid: alist[i].sectionid, section: alist[i].section, AssignSubObj: asssubobj1});
                    }
                }
            }
        },
        ...
        goDetail(aObj){                        
            ...
            for(let i=0; i<aObj.AssignSubObj.length; i++){
                this.AssSubObj.push({id: aObj.AssignSubObj[i].id,itemid: i+1,SubCaption: "Subject",SubValue: aObj.AssignSubObj[i].SubId,TeacherCaption: "Teacher",TeacherValue: aObj.AssignSubObj[i].StaffId});
            }
        ...
        goEdit(aObj){            
            this.$router.push({name: 'editassSubjects', params: {id: aObj[0].id}});
        },
