              ...
              <table class="table table-hover table-striped" id="studenttable">
                ...
                <tbody id="myTable">
                  <tr v-for="classes in ClassList" :key="classes.id" class="active">
                    ...
                    <td class="all" nowrap>
                      <label
                        v-for="section in classes.section"
                        :key="section.section"
  ...
  methods: {
    ...
    check(e, Obj) {
      if (e.target.checked) {
        ...
        this.ClassObj.section.splice(
          this.ClassObj.section.map(item => item).indexOf(Obj.id),
          1
        );
        if (
          this.ClassObj.section == [] ||
          this.ClassObj.section.length == 0 ||
          this.ClassObj.section == undefined
        ) {
      ...
      for (let i = 0; i < this.SectionList.length; i++) {
        ...
        for (let a = 0; a < aObj.section.length; a++) {
          if (this.SectionList[i].section == aObj.section[a].section) {
            editObj.push({
              id: this.SectionList[i].id,
              section: this.SectionList[i].section,
              checked: true
            });
