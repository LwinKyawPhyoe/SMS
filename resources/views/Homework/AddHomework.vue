          ...
          <div class="row" id="row" style="margin: 0px;">
            <div class="col-lg-4 col-12 textbox">
              ...
              <select
                id="class_home_id"
                class="inputbox"
                @change="selectClass(objData.class,'class_home_id','class_home_msg')"
                name="class"
                v-model="objData.class"
              >
            ...
            <div class="col-lg-4 col-12 textbox">
              ...
              <select
                id="section_home_id"
                class="inputbox"
                @change="selectSection(objData.section,'section_home_id','section_home_msg')"
                name="class"
                v-model="objData.section"
              >
            ...
            <div class="col-lg-4 col-12 textbox">
              <label for="name" class="title">Subject</label>
            ...
            <table class="table table-hover table-striped" v-if="homeworkList.length!=0">
              ...
              <tbody>
                <tr class="active" v-for="data in homeworkList">
                  <td :class="arrayTableColumns[0].class" nowrap>{{ data.class }}</td>
                  <td :class="arrayTableColumns[1].class" nowrap>{{ data.section }}</td>
                  <td :class="arrayTableColumns[2].class" nowrap>{{ data.subject }}</td>
                  <td :class="arrayTableColumns[3].class" nowrap>{{ showDate(data.homework_date) }}</td>
                  <td
                    :class="arrayTableColumns[4].class"
                    nowrap
                  >{{ showDate(data.submission_date) }}</td>
                  <td :class="arrayTableColumns[5].class" nowrap>{{ showDate(returnDate(data)) }}</td>
                  <td :class="arrayTableColumns[6].class" nowrap></td>
                  <td :class="arrayTableColumns[7].class" style="text-align:right;min-width: 80px">
                    <i
                      class="fa fa-list pen"
                      data-toggle="modal"
                      data-target="#EvaluateHomework"
                      @click="openDetailHomework(data)"
                    >
                    ...
                    <i
                      ...
                      @click="openEditHomework(data)"
                    >
                      <span class="penLabel">Edit</span>
                    </i>
                    <i
                      class="fa fa-trash time"
                      @click="goDelete(data.homeworkId)"
                      data-toggle="modal"
...
export default {
    ...
    return {
      arrayTableColumns: [
        ...
        { Name: "Subject", Id: "SubjectId", class: "tbl_body_Subject" },
        {
          Name: "Homework Date",
          Id: "HomeworkDateId",
          class: "tbl_body_Homework"
        },
        {
          Name: "Submission Date",
          Id: "SubmissionDateId",
          class: "tbl_body_Submission"
        },
        {
          Name: "Evaluation Date",
          Id: "Evaluation DateId",
          class: "tbl_body_Evaluation"
        },
  ...
  methods: {
    ...
    selectSection(value, inputId, megId) {
      ...
      this.objData.subject = "";
      let data = {
        class_id: this.objData.class,
        section_id: this.objData.section
      };
      this.axios.post("/api/AssSubject/search", data).then(response => {
    ...
    SearchData() {
      if (this.checkValidation()) {
        ...
        if (this.objData.subject == "") {
          this.axios
            .post("/api/homework/searchHomework", formData)
            .then(result => {
              EventBus.$emit("onLoadEnd");
          ...
          this.axios
            .post("/api/homework/searchHomeworkSub", formData)
            .then(result => {
              EventBus.$emit("onLoadEnd");
              this.homeworkList = result.data;
            });
