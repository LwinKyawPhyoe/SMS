          ...
          <div class="row" id="row" style="margin: 0px;">
            <div class="col-lg-4 col-12 textbox">
              ...
              <select
                id="class_id"
                class="inputbox"
                @change="selectClass(objData.class,'class_id','class_msg')"
                name="class"
            ...
            <div class="col-lg-4 col-12 textbox">
              ...
              <select
                id="section_id"
                class="inputbox"
                @change="selectSection(objData.section,'section_id','section_msg')"
                name="class"
            ...
            >
              ...
              <tbody>
                <tr class="active" v-for="data in homeworkList">
                  <td :class="arrayTableColumns[0].class" nowrap>{{data.subject}}</td>
                  <td :class="arrayTableColumns[1].class" nowrap>{{showDate(data.homework_date)}}</td>
                  <td :class="arrayTableColumns[2].class" nowrap>{{showDate(data.submission_date)}}</td>
                  <td :class="arrayTableColumns[3].class" nowrap>{{returnRatio(data)}}</td>
                  <td :class="arrayTableColumns[4].class" nowrap>{{returnPrecent(data)}}</td>
                  <td style="text-align:right;" :class="arrayTableColumns[5].class">
                    <i
                      class="fa fa-list pen"
                      data-toggle="modal"
                      data-target="#EvaluateHomework"
                      @click="openEvaluation(data)"
                    >
    ...
    return {
      ...
      arrayTableColumns: [
        ...
        },
        {
          ...
          Id: "CompleteIncompleteId",
          class: "tbl_body_CompleteIncomplete"
        },
    ...
    SearchData() {
      if (this.checkValidation()) {
        ...
        if (this.objData.subject == "") {
          this.axios
            .post("/api/homework_evaluation/searchEvaluation", formData)
            .then(result => {
          ...
          this.axios
            .post("/api/homework_evaluation/searchEvaluationSub", formData)
            .then(result => {
              this.homeworkList = result.data;
              EventBus.$emit("onLoadEnd");
            });
